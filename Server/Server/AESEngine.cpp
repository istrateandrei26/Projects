...
QByteArray AESEngine::encryptAES(QByteArray passphrase, QByteArray& data)
{
	QByteArray msalt = randomBytes(SALTSIZE);
	int rounds = 1;
	unsigned char key[KEYSIZE];
	unsigned char iv[IVSIZSE];
...
QByteArray AESEngine::decryptAES(QByteArray passphrase, QByteArray& data)
{
	QByteArray msalt;
...
QByteArray AESEngine::randomBytes(int size)
{
...
unique_ptr<ICipherEngine> Cipher_Headquarter::Build_AES_Engine()
{
	return make_unique<AESEngine>();
}